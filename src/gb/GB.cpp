      ...
      for (int g = 0; g < 32; g++)
      {
         for (int b = 0; b < 32; b++)
         {
            ...
            int ng = gbGetValue(gbGetValue(4 + gbGetValue(0, 5, r), 14 + gbGetValue(0, 3, r), b),
                     gbGetValue(24 + gbGetValue(0, 3, r), 29 + gbGetValue(0, 1, r), b), g) - 4;
            int nb = gbGetValue(gbGetValue(4 + gbGetValue(0, 5, r), 14 + gbGetValue(0, 3, r), g),
                     gbGetValue(24 + gbGetValue(0, 3, r), 29 + gbGetValue(0, 1, r), g), b) - 4;
...
void gbWriteMemory(uint16_t address, uint8_t value)
{
   ...
   // GBC allows reading/writing to this area
...
bool gbReadSaveMBC3(const char* name)
{
   ...
   { // read RTC data
      ...
      if (!read || (read != MBC3_RTC_DATA_SIZE && read != MBC3_RTC_DATA_SIZE - 4))
      { // detect old 32 bit saves
...
bool gbReadSaveTAMA5(const char* name)
{
   ...
   else
   {
      ...
      if (!read || (read != TAMA5_RTC_DATA_SIZE && read != TAMA5_RTC_DATA_SIZE - 4))
      { // detect old 32 bit saves
         systemMessage(MSG_FAILED_TO_READ_RTC, N_("Failed to read RTC from save game %s (continuing)"),
             name);
         res = false;
