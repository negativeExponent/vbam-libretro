...
static void gbDrawBG()
{
   ...
   {
      if ((register_LCDC & 0x01 || isCGB) && (layerSettings & 0x0100))
      {
         while (x < 160)
         {
      ...
      // LCDC.0 also enables/disables the window in !isCGB ?!?!
      // (tested on real hardware)
      // This fixes Last Bible II & Zankurou Musouken
      if ((register_LCDC & 0x01 || isCGB) && (register_LCDC & 0x20) && (layerSettings & 0x2000) && (gbWindowLine != -2))
      {
         int i = 0;
...
template <bool isCGB>
static void gbDrawSpriteTile(int tile, int x, int y, int t, int flags,
...
template <bool isCGB>
static void gbDrawSprites(bool draw)
{
   int x = 0;
   int y = 0;
   int count = 0;
   ...
   if ((register_LCDC & 2) && (layerSettings & 0x1000))
   {
      ...
      for (int i = 0; i < 40; i++)
      {
         ...
         if (x > 0 && y > 0 && x < 168 && y < 160)
         {
            ...
            {
               if (draw)
                  gbDrawSpriteTile<isCGB>(tile, x - 8, yc, t, flags, size, i);
               else
               {
...
void gbRenderLine(bool draw)
{
   if (draw)
   {
      if (gbCgbMode)
         gbDrawSprites<true>(true);
      else
         gbDrawSprites<false>(true);
   }
   else
   {
      if (gbCgbMode)
      {
         gbDrawBG<(true)>();
         gbDrawSprites<true>(false);
      }
      else
      {
         gbDrawBG<false>();
         gbDrawSprites<false>(false);
      }
   }
}
